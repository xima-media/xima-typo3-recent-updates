...
function add_typoscript_set() {
    mysql -uroot -proot $DATABASE -e 'UPDATE sys_template SET include_static_file = CONCAT(include_static_file, ",EXT:xima_typo3_recent_updates/Configuration/TypoScript") WHERE uid = 1;'
}
...
function add_site_set() {
  ...
  if ! grep -q "xima/xima-typo3-recent-updates" "$yaml_file"; then
